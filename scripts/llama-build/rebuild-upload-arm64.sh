#!/usr/bin/env bash
# Rebuild Ubuntu 22.04 CUDA arm64 tarball and upload to GitLab generic packages.
set -euo pipefail
...
SRC="${LLAMA_BUILD_WORK_DIR}/src-arm64/llama.cpp"
OUT="${LLAMA_BUILD_OUT_DIR}"
TAR="llama-${TAG}-bin-ubuntu-cuda-arm64.tar.gz"
PKG_ROOT="llama-${TAG}-bin-ubuntu-cuda-arm64"
...
docker run --platform "${PLATFORM}" --pull=never --rm \
  ...
  -v "${ROOT}/build-ubuntu22-cuda-arm64.sh:/build.sh:ro" \
  -v "${OUT}:/out" \
  -e WORKDIR=/work \
  -e OUTDIR=/out \
  -e LLAMA_TAG="${TAG}" \
...
docker run --platform "${PLATFORM}" --pull=never --rm \
  ...
  "mkdir -p /tmp/p && tar -xzf /pkg.tar.gz -C /tmp/p && r=/tmp/p/${PKG_ROOT} && LD_LIBRARY_PATH=\$r/lib \$r/bin/llama-server --version"
