...
$DefaultDistBaseUrl = "https://opencsg-public-resource.oss-cn-beijing.aliyuncs.com/codex-releases"
$DistBaseUrl = if ($env:CSGHUB_LITE_CODEX_DIST_BASE_URL) { $env:CSGHUB_LITE_CODEX_DIST_BASE_URL } else { $DefaultDistBaseUrl }
...
function Install-ExtractedRuntime([string]$Version, [string]$BinaryName, [string]$ExtractedDir) {
    ...
    $runtimeRoot = Join-Path $env:USERPROFILE ".local\share\codex\versions"
    ...
    Write-Output "INFO: installed Codex runtime $Version to $versionDir"
    if ($linked) {
...
if (-not [Environment]::Is64BitProcess) {
    throw "Codex does not support 32-bit Windows."
}

$distBaseUrl = Trim-TrailingSlash $DistBaseUrl
$requestedVersion = Normalize-RequestedVersion $Target
$workDir = Join-Path $env:TEMP ("codex-install-" + [guid]::NewGuid().ToString("N"))
...
try {
    ...
    if ([string]::IsNullOrWhiteSpace($version)) {
        throw "failed to resolve Codex version"
    }
    ...
    Write-Output "INFO: downloading Codex $version for $platform from $distBaseUrl"
